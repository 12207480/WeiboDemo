...
@property (nonatomic, strong) NSDictionary  *viewRectDictionary;
...
- (void)saveTextStorageRectWithFrame:(CTFrameRef)frame
{
    ...
    NSMutableDictionary *viewRectDictionary = [NSMutableDictionary dictionary];
    // 获取每行有多少run
        ...
        for (int j = 0; j < CFArrayGetCount(runs); j++) {
            ...
            if (textStorage) {
                ...
                if ([textStorage conformsToProtocol:@protocol(TYViewStorageProtocol)]) {
                    [viewRectDictionary setObject:textStorage forKey:[NSValue valueWithCGRect:runRect]];
                }else if ([textStorage conformsToProtocol:@protocol(TYDrawStorageProtocol)]) {
    ...
    if (viewRectDictionary.count > 0) {
        _viewRectDictionary = [viewRectDictionary copy];
    }else {
        _viewRectDictionary = nil;
    }
...
{
    return _viewRectDictionary.count != 0;
}

- (void)enumerateViewRectDictionaryUsingBlock:(void (^)(id<TYViewStorageProtocol> drawStorage, CGRect rect))block
{
    [_viewRectDictionary enumerateKeysAndObjectsUsingBlock:^(NSValue *rectValue, id<TYViewStorageProtocol> drawStorage, BOOL * stop) {
        if (block) {
