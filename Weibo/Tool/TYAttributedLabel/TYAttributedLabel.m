...
- (BOOL)existViewRectDictionary;

- (void)enumerateViewRectDictionaryUsingBlock:(void (^)(id<TYViewStorageProtocol> drawStorage, CGRect rect))block;
...
@interface TYAttributedLabel ()<UIGestureRecognizerDelegate, TYAsyncLayerDelegate>
{
    struct {
...
- (void)setAsyncRenderLayerScale
{
    ...
    ((TYAsyncRenderLayer *)self.layer).contentsScale = scale;
}

- (void)setAsynchronousRendering:(BOOL)asynchronousRendering
{
    _asynchronousRendering = asynchronousRendering;
    ((TYAsyncRenderLayer *)self.layer).asynchronousRendering = asynchronousRendering;
}
...
+ (Class)layerClass {
    return [TYAsyncRenderLayer class];
}
...
- (void)setNeedsDisplayOnMain
{
    ((TYAsyncRenderLayer *)self.layer).asynchronousRendering = NO;
     [self.layer setNeedsDisplay];
}
...
{
    // create display task
    TYAsyncLayerRenderTask *task = [[TYAsyncLayerRenderTask alloc]init];
    
    task.willRendering = ^(TYAsyncRenderLayer *layer){
         [layer removeAnimationForKey:@"contents"];
        if (layer.asynchronousRendering) {
            layer.contents = nil;
        }
        [self removeViewStorage];
    };
    
    task.rendering = ^(CGContextRef context, CGSize size, BOOL (^isCancelled)(void)) {
        if (isCancelled() ||!context || _textContainer == nil ||  _textContainer.attString == nil) return;
        
        [_textContainer createTextContainerWithContentSize:self.bounds.size];
        
        [self transformTextContext:context size:size];
        if (isCancelled()) return;
        [self drawHighlightBackgroundInContext:context];
        
        if (isCancelled()) return;
        // CTFrameDraw 将 frame 描述到设备上下文
        [self drawText:_textContainer.attString frame:_textContainer.frameRef size:size context:context isCancelled:isCancelled];
        
        if (isCancelled()) return;
        // 画其他元素
        [self drawTextStorageInContext:context isCancelled:isCancelled];
    };
    
    task.didRendering = ^(TYAsyncRenderLayer *layer, BOOL finished){
        if (!finished) {
            [layer removeAnimationForKey:@"contents"];
            [self removeViewStorage];
            layer.asynchronousRendering = _asynchronousRendering;
            return ;
        }
        [layer removeAnimationForKey:@"contents"];
        [self drawViewStorage];
        [self addGestureRecognizersIfNeed];
...
//    
//    [self transformTextContext:context size:rect.size];
//    
//    [self drawHighlightBackgroundInContext:context];
//    
//    // CTFrameDraw 将 frame 描述到设备上下文
//     [self drawText:_textContainer.attString frame:_textContainer.frameRef size:rect.size context:context];
//    
//    // 画其他元素
//    [self drawTextStorageInContext:context];
//    [self drawViewStorage];
//    
//    // 添加手势
//    [self addGestureRecognizersIfNeed];
...
- (void)transformTextContext:(CGContextRef)context size:(CGSize)size
{
    // 文本垂直对齐方式位移
    CGFloat verticalOffset = 0;
    switch (_verticalAlignment) {
        case TYVerticalAlignmentCenter:
            verticalOffset = MAX(0, (size.height - _textContainer.textHeight)/2);
            break;
        case TYVerticalAlignmentBottom:
            verticalOffset = MAX(0, (size.height - _textContainer.textHeight));
            break;
        default:
            break;
    }
...
- (void)drawHighlightBackgroundInContext:(CGContextRef)context
{
    if (_highlightedLinkBackgroundColor && [_textContainer existLinkRectDictionary]) {
        [self drawSelectionAreaFrame:_textContainer.frameRef InRange:_clickLinkRange radius:_highlightedLinkBackgroundRadius bgColor:_highlightedLinkBackgroundColor context:context];
    }
}
...
//    {
        ...
        NSInteger numberOfLines = _textContainer.numberOfLines == 0 ? CFArrayGetCount(lines) : MIN(_textContainer.numberOfLines, CFArrayGetCount(lines));
        ...
        for (CFIndex lineIndex = 0; lineIndex < numberOfLines; lineIndex++)
        {
            ...
            if (lineIndex == numberOfLines - 1 && truncateLastLine)
            {
                ...
                if (lastLineRange.location + lastLineRange.length < attributedString.length)
                {
                    ...
                    CTLineRef truncatedLine = CTLineCreateTruncatedLine(truncationLine, size.width, truncationType, truncationToken);
                    if (!truncatedLine)
                    {
...
- (void)drawTextStorageInContext: (CGContextRef)context isCancelled:(BOOL (^)(void))isCancelled
{
    // draw storage
    [_textContainer enumerateDrawRectDictionaryUsingBlock:^(id<TYDrawStorageProtocol> drawStorage, CGRect rect) {
        ...
        [drawStorage drawStorageWithRect:rect context:context];
    }];
}

- (void)drawViewStorage
{
    [_textContainer enumerateViewRectDictionaryUsingBlock:^(id<TYViewStorageProtocol> viewStorage, CGRect rect) {
        [viewStorage setOwnerView:self];
        rect = UIEdgeInsetsInsetRect(rect,viewStorage.margin);
        [viewStorage drawStorageWithRect:rect context:nil];
    }];
}

- (void)removeViewStorage
{
    [_textContainer enumerateViewRectDictionaryUsingBlock:^(id<TYViewStorageProtocol> viewStorage, CGRect rect) {
        [viewStorage setOwnerView:nil];
    }];
}

- (void)addGestureRecognizersIfNeed
{
...
// 绘画选择区域
- (void)drawSelectionAreaFrame:(CTFrameRef)frameRef InRange:(NSRange)selectRange radius:(CGFloat)radius bgColor:(UIColor *)bgColor context:(CGContextRef)context{
    ...
    for (int i = 0; i < count; i++) {
        ...
        if ([self isPosition:selectionStartPosition inRange:range] && [self isPosition:selectionEndPosition inRange:range]) {
            ...
            [self fillSelectionAreaInRect:lineRect radius:radius bgColor:bgColor context:context];
            break;
        }
...
- (void)fillSelectionAreaInRect:(CGRect)rect radius:(CGFloat)radius bgColor:(UIColor *)bgColor context:(CGContextRef)context {
